# solver-sm-radical-v6-n54: a second, stronger AI-written solver

A **distinct** AI-written circle-packing solver from a _different_
self-improvement run than the top-level
[`solver-sm-radical-v6-n27/`](../solver-sm-radical-v6-n27/). Where
`solver-sm-radical-v6-n27/` swept N=2..100 and won at **N=27** but
under-searched at large N (it falls short of the record at, e.g., N=54), this
solver **beats the Packomania `csqv` best-known exactly there**: **N=54
(+9.0e-4)** and **N=55 (+2.3e-5)**.

## Provenance

Generated by the SI-v2 self-improvement loop (not hand-written):

- **Run:** `circle-packing-n54-240s-20260809`, values `sm-radical-v6`, **stopped
  at iteration 10**.
- **Source commit:** `06a8c0a` (iteration-10 solver). Verified Σr(n=54) =
  **3.8426357949378** vs record 3.841733103296, feasible at zero tolerance
  (independent recheck).
- Different lineage and codebase from `solver-sm-radical-v6-n27/` (which uses
  `pack.py`/`container.py`/`shape.py` and a `.pck` sweep harness); this one is
  the n-generic **SLP-KKT pipeline**.
...
`sota/sm-radical-v6-n54/`.)

## Status & how it would slot into the comparison

This directory is the **solver only**. A full N-sweep of it (analogous to the
top-level `reproduce.sh` sweep of `solver-sm-radical-v6-n27/`) has **not** been
run here yet; when it is, its results/packings belong in a sibling
`sota/sm-radical-v6-n54/` so both solvers are compared against the same
`sota/packomania/` records.
...
a **pure linear program in `r`**: exact, no gradient noise, no step-size tuning.
Both solvers implement it with scipy/HiGHS and fall back to a numpy-only
...
[n27 solver's algorithm](../solver-sm-radical-v6-n27/README.md#algorithm)).

### The updated solver (`solver-sm-radical-v6-n54/`): the 21 live wins
