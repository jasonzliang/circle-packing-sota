...
WORKERS="${4:-${WORKERS:-8}}"; SEEDS="${SEEDS:-1}"; OUT="repro"    # fresh dir; does NOT clobber the committed sota/sm-radical-v6-n27 reference
...
OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1 \
  python3 solver-sm-radical-v6-n27/run_sweep.py --nmin "$NMIN" --nmax "$NMAX" --time "$TIME" \
