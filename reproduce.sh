...
WORKERS="${4:-${WORKERS:-8}}"; SEEDS="${SEEDS:-1}"; OUT="repro"    # fresh dir; does NOT clobber the committed sota/ours reference
...
OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1 \
  python3 solver/run_sweep.py --nmin "$NMIN" --nmax "$NMAX" --time "$TIME" \
          --workers "$WORKERS" --seeds "$SEEDS" --out-dir "$OUT"

echo "== compare to Packomania csqv records -> $OUT/comparison.md =="
python3 solver/compare.py --results "$OUT/results.csv" --out "$OUT/comparison.md" | tail -n +1 | sed -n '1,6p'

echo "== independently verify every emitted packing (pure-stdlib, no solver) =="
fail=0
for f in "$OUT"/pck/csqv*.pck; do
  python3 solver/verify_pck.py "$f" >/dev/null || { echo "  INFEASIBLE: $f"; fail=1; }
done
