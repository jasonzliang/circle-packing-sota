...
**[Jason Liang's homepage](https://jasonzliang.github.io/)**
...
[Citation](#citation) · [Latest release](https://github.com/jasonzliang/circle-packing-sota/releases/latest)
...
containment, overlaps and Σr from the coordinates alone. Dated snapshots from
Packomania preserve the comparisons reported here. Fetch a fresh table to
check current standings, since the `csqv` best-known values change over time.
...
Σr = 2.685978684198) **matches** the best-known value in the August 10, 2026
snapshot to 12 digits. Its status against a fresh table may differ. Against
the older 2011/12 value, it shows `BEATS`:
...
<a id="reproduce-the-21-live-record-beats-n54-solver-the-headline"></a>

## Reproduce the 21 improvements (n54 solver)

**No search needed to check them**: the 21 wins are fixed `.pck` files.
To reproduce the dated comparison from a fresh checkout, use the stored
August 10, 2026 reference table:

```bash
python3 verify_and_compare.py compare --pck-dir sota/sm-radical-v6-n54 \
  --records sota/packomania/packomania_csqv.json --out comparison-n54.md
```

This re-verifies all 92 committed packings and prints `WINS: 21`. Adding
`--refresh` checks current standings instead; the count can change as the
reference records do.
...
solver-sm-radical-v6-n54/  pipeline.py slp.py packlib.py endgame.py broad.py ...  # updated solver (21 wins vs. 2026-08-10)
...
sota/          the SOTA comparison, all in one place:
  packomania/    packomania_csqv.json (refreshable) + history/<dated> snapshots + README
...
**The updated n54 solver** (`solver-sm-radical-v6-n54/`) produced the **21
improvements against the August 10, 2026 snapshot**. Its n-generic entry point
`solve(n, seconds, seed)`
...
best-known (Σr = 2.685978684198, matching the August 10, 2026 snapshot; see
...
is the **autonomous synthesis plus independently verified improvements**,
not a novel method.
