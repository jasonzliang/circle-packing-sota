...
the Packomania `csqv` best-known values in [`../packomania/`](../packomania/). The complete per-N packings from
...
- Strictly feasible (independently re-checked with `verify_and_compare.py verify`, below):
...
**What the seed does.** Inside `solver-nietzsche-sm-radical-v6-n27/pack.py`, `search(n, seed, budget)` creates **one** numpy PCG64
